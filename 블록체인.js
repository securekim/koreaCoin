...
const 시간이올바른지 = (새로운블록, 이전블록) => {
  ...
  const 체인이올바른지 = 후보체인 => {
    ...
        return null;}
        // 후보체인을 돌면서 이전 블록과 비교해 검증.
        for (let 번째 = 0; 번째 < 후보체인.length; 번째++) {
          const 현재블록 = 후보체인[번째];
          if (번째 !== 0 && !블록이올바른지(현재블록, 후보체인[번째 - 1])) {
            return null;
          }
        }
        return true;
...
const 블록자동채굴 = () => {
      //TODO : CoinbaseTx
        console.log("채굴 !", 최신블록가져오기().인덱스 + 1, " 난이도 : ",최신블록가져오기().난이도);
        return 블록채굴({});
};
